...
`install.packages()` is used to install the dependencies/packages we
need and `library()` is used to load those dependencies.
`install.packages()` is called only once when we install the packages
initially, whereas, `library()` is called every time we start a new
analysis in R to load the packages.
