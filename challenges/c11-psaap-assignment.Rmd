...
-   The dataset has 22 columns (features) and 140 observations (data points).
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   The most accurate model is the Cheat model (`avg_T`), with the lowest RMSE of 0.2318.
    -   The least accurate model is the Non-physical model (`idx`), with the highest RMSE of 0.3988.
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   `avg_T` is an Output variable.

        It's so effective because `T_norm` is calculated directly from it: T_norm = (avg_T - T_f) / T_f.

    -   ​So `avg_T` almost directly determines the target, making it a near-perfect predictor (but also a cheat).
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   No, we would not have access to `avg_T` when predicting `T_norm` for a new scenario.
    -   It’s an output variable, derived from the simulation itself, not an input we control or measure ahead of time.
    -   Thus, `avg_T` is not a valid predictor in practice — including it would be like using the answer to predict itself.
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   `idx` is a Metadata variable.
    -   It identifies the simulation run, but it has no physical meaning as it's just a label or ID.\
        Using it in modeling risks overfitting to arbitrary run IDs without generalizing to new simulations.
...
-   Which columns are excluded in the model formula above? What categories do these belong to? Why are these important quantities to leave out of the model?
    ...
    -   Why exclude them:
        -   `avg_T`, `avg_q`, `rms_T` are output variables that depend on the simulation result, and including them would leak information from the result we're trying to predict, defeating the purpose of modeling.

        -   `idx` is a metadata label, not a physical quantity — it provides no generalizable predictive power and may lead to overfitting.
-   Which inputs are *statistically significant*, according to the model?
    -   W (channel width) and T_f (fluid inlet temperature) are statistically significant predictors.
-   What is the regression coefficient for `x`? What about the regression coefficient for `T_f`?
    -   Only `T_f` is in the model:
        ...
        -   `x` is not in the model as it was excluded intentionally to isolate effects of upstream input variables only.
...
-   How do these standard deviations relate to the regression coefficients for `x` and `T_f`?
    -   The regression coefficient for `T_f` is -0.004783:\
        ...
        Since `T_f` has large natural variation (SD \~39), this effect becomes more impactful in practice.
    -   `x` isn't included in this model, but in earlier models (like `fit_baseline`), its effect could be interpreted relative to the SD of `x` to compare predictive strength.
-   Note that literally *all* of the inputs above have *some* effect on the output `T_norm`; so they are all "significant" in that sense. What does this tell us about the limitations of statistical significance for interpreting regression coefficients?
    -   A variable may have real physical importance but appear not statistically significant due to:
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   The `fit_q4` model (left side) is more accurate than the `fit_simple` (`x only`) model. I can tell because:

        -   Points are closer to the diagonal 1:1 line (ideal prediction line).
        -   The vertical error bars (difference between predicted and actual) are generally shorter.

        <!-- -->

        -   The spread around the line is tighter, indicating better alignment between predicted and actual `T_norm`.
-   Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
    -   The `fit_q4` model is also more confident as its prediction intervals are narrower. You can see this in the shorter vertical error bars in the left panel (`model: q4`) compared to the right panel (`model: x only`).
-   How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
    -   The **`fit_simple` model** only needs **1 predictor**: `x` (channel location).
    -   The **`fit_q4` model** uses **6 predictors**: `L`, `W`, `U_0`, `N_p`, `k_f`, `T_f`.
...
-   How much do you trust your model? Why?
    -   I moderately trust the model because it is trained on a reasonably sized dataset with physically meaningful predictors, but the wide prediction interval (from about −0.89 to 2.68) suggests that significant uncertainty remains due to uncontrolled variables.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   A prediction interval is appropriate, because we want to estimate the range of possible `T_norm` outcomes at the design point, not just the average. The team needs to design for individual outcomes, not for the mean behavior.
-   What fraction of validation cases lie within the intervals you predict? (NB. Make sure to calculate your intervals *based on the validation data*; don't just use one single interval!) How does this compare with `pr_level`?
    -   65% of validation cases fall within the predicted 80% intervals, which suggests that the interval estimates are well calibrated and trustworthy for decision-making.
-   What interval for `T_norm` would you recommend the design team to plan around?
    -   I recommend the design team plan around a T_norm range of approximately −0.89 to 2.68, which captures 80% of expected outcomes under their proposed design. This will ensure safe and efficient operation under most likely conditions.
-   Are there any other recommendations you would provide?
    -   Consider constraining or monitoring additional variables (examples: `T_f`, `N_p`, `I_0`) to reduce uncertainty.
    -   Avoid operating close to system thresholds if this range could exceed safety limits.
    -   If precision is critical, consider adding sensors or controls for the most influential uncontrolled variables.
