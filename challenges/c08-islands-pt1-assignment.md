...
- Are there any sources of *real* uncertainty in the percent occupied
  you calculated?
  - I think there could be real uncertainty if the homes are occupied on
    paper but no one is living there physically.
- Are there any sources of *erroneous* uncertainty in the percent
  occupied you calculated?
  - There could be erroneous uncertainty if I miscounted the number of
    unoccupied homes as I subtracted that figure from the total number
    of homes to calculate the percent occupied.
...
``` r
## Compute the prevalence and sort
df_q3 <-
  df_sample_seq %>%
  mutate(
    last_name = sapply(
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - No, the sample is not representative of all houses in Helvig because
    the dataset contains the data of just the individuals living in the
    first 25 houses.
...
```
    ...
    ## ── Column specification
    ## ──────────────────────────────────────────────────────── Delimiter: "," chr
    ## (1): name dbl (2): house, age lgl (1): ...4
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - The random sample is more representative of all homes in Helvig.
  - We’ll get a better data sample by using random sample because each
    house has an equal chance of getting selected and would result in an
    unbiased dataset. On the other hand, the sequential sample selects
    the first 25 houses and this data sample could be biased.
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - Collins, Wilson, Regan
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - Yes, most people with the same name might belong to the same family
    and would be living in the same house. There could also be the case
    that extended families (with the same last name) live in multiple
    houses near to each other.
...
```
    ...
    ## Warning: There were 123 warnings in `mutate()`.
    ...
    ## ℹ Run `dplyr::last_dplyr_warnings()` to see the 122 remaining warnings.
...
```
    ...
    ## 1 Eklund 0.0172    0.0395 0.0862   0.05 percentile
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No, we cannot use the bootstrap to make a confidence interval with
    df_sample_seq that represents all of Helvig because the sample is
    not randomly selected—it only includes individuals from the first 25
    houses. Since bootstrap resampling assumes the original sample is
    representative, using a biased sample would only reinforce its
    existing limitations rather than generalize to the entire
    population.
...
- We will survey residents of one city in The Islands.
...
- (How will you choose your sample size?)
  - We aim for a minimum sample size of 100 to ensure statistical
    reliability while keeping data collection feasible.
  - If variability is high, we may expand the sample size to increase
